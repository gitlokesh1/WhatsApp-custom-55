...
func ensureConversationPipelineTables() error {
    ...
    if conversationSchemaReady { return nil }
    if userDB == nil { return context.Canceled }
    _, err := userDB.Exec(`
...
func normalizeInboundText(value string) string { return strings.Join(strings.Fields(strings.TrimSpace(value)), " ") }

func inboundMessageText(message *waProto.Message) string {
    if message == nil { return "" }
    if text := message.GetConversation(); text != "" { return text }
    if extended := message.GetExtendedTextMessage(); extended != nil { return extended.GetText() }
    return ""
}

func conversationTarget(event *events.Message) (types.JID, bool) {
    if event == nil { return types.JID{}, false }
    chat := event.Info.Chat
    if chat.IsEmpty() { chat = event.Info.Sender }
    if chat.Server == types.HiddenUserServer {
        if !event.Info.SenderAlt.IsEmpty() { chat = event.Info.SenderAlt } else if event.Info.Sender.Server == types.DefaultUserServer { chat = event.Info.Sender }
    }
    if chat.IsEmpty() || chat.Server != types.DefaultUserServer || strings.TrimSpace(chat.User) == "" { return types.JID{}, false }
    return chat, true
}

func conversationCommand(body string) string { return strings.ToUpper(strings.Trim(strings.TrimSpace(body), ".,!?")) }

func recordInboundConversation(accountKey string, target types.JID, messageID, body string) (string, bool, error) {
    if err := ensureConversationPipelineTables(); err != nil { return "", false, err }
    tx, err := userDB.Begin()
    ...
    if err != nil { return "", false, err }
    if rows == 0 { return "", true, nil }
    if _, err = tx.Exec(`INSERT INTO public.conversations(account_key,chat_jid,consent_status,updated_at) VALUES($1,$2,'active',now()) ON CONFLICT(account_key,chat_jid) DO NOTHING`, accountKey, chat); err != nil { return "", false, err }
    var consent string
    var handoff bool
    if err = tx.QueryRow(`SELECT consent_status,human_handoff FROM public.conversations WHERE account_key=$1 AND chat_jid=$2 FOR UPDATE`, accountKey, chat).Scan(&consent, &handoff); err != nil { return "", false, err }
    command := conversationCommand(body)
    reply := ""
    switch command {
    case "STOP", "UNSUBSCRIBE", "OPT OUT", "OPTOUT":
        consent = "opted_out"; handoff = false; reply = "You’re unsubscribed from automated messages. Reply START if you want to resume."
    case "START", "SUBSCRIBE":
        consent = "active"; handoff = false; reply = "Automated replies are enabled again. How can I help?"
    case "HELP":
        if consent != "opted_out" { consent = "active"; reply = "I can help with your request. Reply HUMAN if you want a person to join the conversation." }
    case "HUMAN", "AGENT", "PERSON":
        if consent != "opted_out" { consent = "active"; handoff = true; reply = "I’ll hand this conversation to a person. Please wait for a reply." }
    default:
        if consent != "opted_out" && !handoff { consent = "active"; reply = "Thanks for your message. I’ve received it and will get back to you shortly." }
    }
    if _, err = tx.Exec(`UPDATE public.conversations SET consent_status=$3,human_handoff=$4,last_inbound_at=now(),updated_at=now() WHERE account_key=$1 AND chat_jid=$2`, accountKey, chat, consent, handoff); err != nil { return "", false, err }
...
func processConversationQueue() {
    if err := ensureConversationPipelineTables(); err != nil { return }
    ...
    if err != nil { return }
    type queuedReply struct { id int64; accountKey, chat, body, providerID string }
    var queue []queuedReply
    for rows.Next() { var item queuedReply; if rows.Scan(&item.id,&item.accountKey,&item.chat,&item.body,&item.providerID)==nil { queue=append(queue,item) } }
    rows.Close()
    for _, item := range queue {
        ...
        if err != nil { continue }
        n, _ := claimed.RowsAffected(); if n == 0 { continue }
        session := getSession(item.accountKey)
        if session == nil || session.client == nil || !session.client.IsLoggedIn() || !session.client.IsConnected() { _,_=userDB.Exec(`UPDATE public.conversation_messages SET status='queued',error=$1,next_attempt_at=now()+interval '30 seconds' WHERE id=$2`,"WhatsApp account is not connected",item.id); continue }
        target, err := types.ParseJID(item.chat)
        if err != nil { _,_=userDB.Exec(`UPDATE public.conversation_messages SET status='failed',error=$1 WHERE id=$2`,err.Error(),item.id); continue }
        if item.providerID == "" { item.providerID=string(session.client.GenerateMessageID()); _,_=userDB.Exec(`UPDATE public.conversation_messages SET provider_message_id=$1 WHERE id=$2`,item.providerID,item.id) }
        session.mu.Lock(); sendErr:=safeSendMessageWithID(item.accountKey,session.client,target,item.body,types.MessageID(item.providerID)); session.mu.Unlock()
        if sendErr != nil { if conversationTemporaryError(sendErr) { _,_=userDB.Exec(`UPDATE public.conversation_messages SET status='queued',error=$1,next_attempt_at=now()+interval '90 seconds' WHERE id=$2`,sendErr.Error(),item.id) } else { _,_=userDB.Exec(`UPDATE public.conversation_messages SET status='failed',error=$1 WHERE id=$2`,sendErr.Error(),item.id) }; continue }
...
func conversationWorkerLoop() { for userDB==nil { time.Sleep(2*time.Second) }; for { processConversationQueue(); time.Sleep(2*time.Second) } }
...
func sendTextMessage(ctx context.Context, client *whatsmeow.Client, target types.JID, text string, messageID types.MessageID) error {
    ...
    if messageID=="" { _,err:=client.SendMessage(ctx,target,message); return err }
    _,err:=client.SendMessage(ctx,target,message,whatsmeow.SendRequestExtra{ID:messageID}); return err
}
