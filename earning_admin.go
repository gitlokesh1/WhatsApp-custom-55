...
func adminCountriesHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		...
		for rows.Next() {
			var c countryConfig
			var users, tasks int
			if rows.Scan(&c.Code, &c.Name, &c.CurrencyCode, &c.Timezone, &c.Reward, &c.DailyGoal, &c.Active, &c.DisplayOrder, &users, &tasks) == nil {
				out = append(out, map[string]any{"code": c.Code, "name": c.Name, "currency_code": c.CurrencyCode, "timezone": c.Timezone, "reward_per_message": c.Reward, "daily_goal": c.DailyGoal, "active": c.Active, "display_order": c.DisplayOrder, "users": users, "tasks": tasks})
			}
		}
	...
	if err == nil && oldCurrency != in.CurrencyCode {
		var credits int
		_ = userDB.QueryRow(`SELECT count(*) FROM public.earning_ledger WHERE country_code=$1`, in.Code).Scan(&credits)
		if credits > 0 {
			userFeaturesJSON(w, 409, map[string]any{"status": "error", "message": "Currency cannot change after rewards have been credited"})
			return
		}
	}
	_, err = userDB.Exec(`INSERT INTO public.earning_countries(code,name,currency_code,timezone,reward_per_message,daily_goal,active,display_order,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,now()) ON CONFLICT(code) DO UPDATE SET name=excluded.name,currency_code=excluded.currency_code,timezone=excluded.timezone,reward_per_message=excluded.reward_per_message,daily_goal=excluded.daily_goal,active=excluded.active,display_order=excluded.display_order,updated_at=now()`, in.Code, in.Name, in.CurrencyCode, in.Timezone, in.Reward, in.DailyGoal, in.Active, in.DisplayOrder)
	if err != nil {
		userFeaturesJSON(w, 500, map[string]any{"status": "error", "message": err.Error()})
		return
	}
...
func adminUsersHandler(w http.ResponseWriter, r *http.Request) {
	...
	userFeaturesJSON(w, 200, map[string]any{"status": "success", "users": out, "country_counts": stats})
}
...
func adminUserActionHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "country":
		...
		var credits int
		if err = userDB.QueryRow(`SELECT count(*) FROM public.earning_ledger WHERE user_id=$1::uuid`, in.UserID).Scan(&credits); err != nil {
			userFeaturesJSON(w, 500, map[string]any{"status": "error"})
			return
		}
		if credits > 0 {
