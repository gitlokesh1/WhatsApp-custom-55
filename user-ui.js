(() => {
  ...
  function shell(page) {
    ...
    if (page === 'profile') { const footer = document.createElement('footer'); footer.className = 'u-user-footer'; footer.innerHTML = `<span><strong>88Task</strong> · Verified task rewards</span><button class="u-btn u-footer-logout" type="button">${icon('logout')}Sign out</button>`; main.appendChild(footer); footer.querySelector('button').addEventListener('click', logout); }
  }
