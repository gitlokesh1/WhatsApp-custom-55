(() => {
  ...
  function shell(page) {
    ...
    shellRoot.innerHTML = `<aside class="u-sidebar"><a class="u-brand" href="/dashboard"><span class="u-brand-mark">88</span><span><strong>88Task</strong><small>Earn with every task</small></span></a><nav class="u-nav" aria-label="Primary navigation">${links}</nav><div class="u-sidebar-foot"><p>${escapeHTML(translate('Complete tasks. Build your streak. Grow your rewards.'))}</p><button class="u-logout" type="button">${icon('logout')}<span>Sign out</span></button></div></aside><section class="u-workspace"><header class="u-topbar" role="banner"><div class="u-topbar-copy"><span class="u-brand-mark u-topbar-logo">88</span><div><strong id="uTopGreeting">${headerGreeting}</strong><span>Secure messaging task rewards</span></div></div><div class="u-topbar-actions"><select class="u-language" data-user-language aria-label="Language"></select><a class="u-wallet-pill" href="/wallet" aria-label="Open wallet">${icon('wallet')}<span id="uTopBalance">${headerBalance}</span></a><button class="u-top-logout" type="button" aria-label="Sign out" title="Sign out">${icon('logout')}</button></div></header></section><nav class="u-mobile-nav" aria-label="Mobile navigation">${links}</nav>`;
