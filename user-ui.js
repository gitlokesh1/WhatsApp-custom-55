(() => {
  const iconPaths = {
    ...
    coins: '<ellipse cx="9" cy="7" rx="5" ry="2.5"/><path d="M4 7v4c0 1.4 2.2 2.5 5 2.5s5-1.1 5-2.5V7M4 11v4c0 1.4 2.2 2.5 5 2.5 1.1 0 2.1-.2 2.9-.5"/><path d="M14 12.5c.8-.6 1.9-1 3-1 2.8 0 5 1.1 5 2.5s-2.2 2.5-5 2.5-5-1.1-5-2.5c0-.6.4-1.1 1-1.6M12 14v4c0 1.4 2.2 2.5 5 2.5s5-1.1 5-2.5v-4"/>',
    taskStart: '<rect x="4" y="3" width="16" height="18" rx="3"/><path d="M9 3.5h6M8 9h4M8 14h2"/><path d="m14 12 4 2.5-4 2.5v-5Z"/>',
    invite: '<circle cx="9" cy="8" r="3"/><path d="M3.5 19a5.5 5.5 0 0 1 11 0M17 7v6M14 10h6"/><path d="M16 17.5c1.4-1.8 3.8-.8 3.8 1 0 1.5-1.8 2.6-3.8 3.5-2-.9-3.8-2-3.8-3.5 0-1.8 2.4-2.8 3.8-1Z"/>',
    cashOut: '<path d="M3 6.5h13a2 2 0 0 1 2 2v3M3 6.5V18a2 2 0 0 0 2 2h8"/><path d="M3 6.5 5 3h12M14 15h8M19 12l3 3-3 3"/><circle cx="8" cy="13" r="2"/>',
    calendarCheck: '<rect x="3" y="5" width="18" height="16" rx="3"/><path d="M7 3v4M17 3v4M3 10h18M7.5 15l2 2 4-4"/>',
    levelBadge: '<path d="M12 3 20 6v5c0 5-3.2 8.4-8 10-4.8-1.6-8-5-8-10V6l8-3Z"/><path d="m12 8 1.2 2.4 2.7.4-2 1.9.5 2.7-2.4-1.3-2.4 1.3.5-2.7-2-1.9 2.7-.4L12 8Z"/>',
    receiptHistory: '<path d="M5 3h12a2 2 0 0 1 2 2v7M5 3v18l3-2 3 2 3-2 2 1.3"/><path d="M8 8h7M8 12h4"/><circle cx="18" cy="17" r="4"/><path d="M18 15v2l1.5 1"/>',
  ...
  function shell(page) {
    ...
    shellRoot.innerHTML = `<aside class="u-sidebar"><a class="u-brand" href="/dashboard"><span class="u-brand-mark">88</span><span><strong>88Task</strong><small>Earn with every task</small></span></a><nav class="u-nav" aria-label="Primary navigation">${links}</nav><div class="u-sidebar-foot"><p>${escapeHTML(translate('Complete tasks. Build your streak. Grow your rewards.'))}</p><button class="u-logout" type="button">${icon('logout')}<span>Sign out</span></button></div></aside><section class="u-workspace"><header class="u-topbar" role="banner"><div class="u-topbar-copy"><span class="u-brand-mark u-topbar-logo">88</span><div><strong id="uTopGreeting">${headerGreeting}</strong><span>Secure WhatsApp task rewards</span></div></div><div class="u-topbar-actions"><select class="u-language" data-user-language aria-label="Language"></select><a class="u-wallet-pill" href="/wallet" aria-label="Open wallet">${icon('wallet')}<span id="uTopBalance" class="u-shell-balance">${headerBalance}</span></a><button class="u-top-logout" type="button" aria-label="Sign out" title="Sign out">${icon('logout')}</button></div></header></section><nav class="u-mobile-nav" aria-label="Mobile navigation">${links}</nav>`;
    ...
    window.uInitLanguageSelectors?.(shellRoot); window.uTranslate?.(shellRoot);
  }
  ...
  function celebrate(reward, currency, gamification) {
    ...
    const streakTemplate = streakDays === 1 ? 'Task verified. Your streak is now 1 day.' : 'Task verified. Your streak is now {count} days.';
